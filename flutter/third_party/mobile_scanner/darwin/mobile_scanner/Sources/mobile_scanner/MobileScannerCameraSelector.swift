...
class MobileScannerCameraSelector {
    ...
    ///
    /// On macOS this includes external/USB cameras (e.g. webcams),
    /// which are not discoverable through `selectCamera`.
    static func availableDevices() -> [AVCaptureDevice] {
        return AVCaptureDevice.devices(for: .video)
    }

    /// Find a video capture device by its unique identifier.
