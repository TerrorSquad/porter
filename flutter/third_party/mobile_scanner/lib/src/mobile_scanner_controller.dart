...
class MobileScannerController extends ValueNotifier<MobileScannerState> {
  ...
  /// The id of a specific camera device to use.
  ///
  /// Currently only supported on macOS, where it allows selecting an
  /// external camera (e.g. a USB webcam) instead of the built-in camera.
  /// Can be changed before calling [start] to select a different camera.
  String? cameraId;
  ...
  ///
  /// Each entry is a map with an `id` and a `label`, identifying a video
  /// capture device. The `id` can be assigned to [cameraId] before calling
  /// [start] to select that camera.
  ///
  /// Currently only supported on macOS, where it includes external/USB
  /// cameras. Returns an empty list on other platforms.
  ///
  /// This method can be called before starting the scanner.
  ///
  /// Throws a [MobileScannerException] if the controller has been disposed.
  Future<List<Map<String, String>>> getAvailableCameras() async {
    if (_isDisposed) {
      throw MobileScannerException(
        errorCode: MobileScannerErrorCode.controllerDisposed,
        errorDetails: MobileScannerErrorDetails(
    ...
    final platform = MobileScannerPlatform.instance;

    if (platform is MethodChannelMobileScanner) {
      return platform.getAvailableCameras();
    }
