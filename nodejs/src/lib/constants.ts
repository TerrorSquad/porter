...
// EC Level H
export const qrCapacityH = [
  7, 14, 24, 34, 44, 58, 64, 84, 98, 119, 137, 155, 177, 194, 220, 250, 280, 310, 338, 382, 403, 439, 461, 514, 535, 593, 625, 658, 698, 742, 790, 842, 898, 958, 983, 1051, 1093, 1139, 1219, 1273 - 100 // Safe margin for V40 H
];

export function getMaxCapacity(version: number, ecc: 'L' | 'M' | 'Q' | 'H'): number {
  const index = Math.max(1, Math.min(40, version)) - 1;
  switch (ecc) {
    case 'L': return qrCapacityL[index];
    case 'M': return qrCapacityM[index];
    case 'Q': return qrCapacityQ[index];
    case 'H': return qrCapacityH[index];
    default: return qrCapacityL[index];
  }
}
