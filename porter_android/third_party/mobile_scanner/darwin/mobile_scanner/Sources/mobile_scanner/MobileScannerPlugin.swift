...
public class MobileScannerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func start(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        // On macOS, a specific camera (e.g. an external USB webcam) can be
        // requested by id. Falls back to the built-in camera otherwise.
        let cameraId = argReader.string(key: "cameraId")
        if let cameraId = cameraId, let requestedDevice = MobileScannerCameraSelector.device(withId: cameraId) {
...
#if os(macOS)
                    deviceAnswer["numberOfCameras"] = MobileScannerCameraSelector.availableDevices().count
#endif
                    answer = deviceAnswer
                } else {
                    answer = [
    ...
    /// List all available video capture devices (id + display name).
    private func getAvailableCameras(_ result: @escaping FlutterResult) {
        let cameras = MobileScannerCameraSelector.availableDevices().map { device -> [String: Any] in
            return [
                "id": device.uniqueID,
