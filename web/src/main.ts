...
const camWrap     = document.getElementById('camera-wrap')    as HTMLDivElement;
...
function rerenderTransfers(): void {
  const transfers = Array.from(assembler.getTransfers().values())
...
function cardHtml(t: Transfer): string {
  ...
  const checksumBadge = t.verified === true
  ...
  const preview = (t.assembled && t.mode === 'T')
    ? (() => {
  ...
  const dlBtn = (t.assembled && !t.error)
  ...
  const relayRow = (() => {
    if (!relayUrl()) return '';
    if (!rs) return `<div class="relay-row relay-row-pending">○ Go receiver: waiting for first chunk…</div>`;
    if (rs.lastError) return `<div class="relay-row relay-row-err" title="${escHtml(rs.lastError)}">✕ Go receiver: error — ${escHtml(rs.lastError.slice(0, 80))}</div>`;
    if (rs.complete && rs.joinedPath) return `<div class="relay-row relay-row-ok">✓ Go receiver: joined → <code>${escHtml(rs.joinedPath)}</code></div>`;
...
function triggerDownload(t: Transfer): void {
  ...
  const blob = new Blob([t.assembled as unknown as Uint8Array<ArrayBuffer>], { type: guessMime(t) });
...
function guessMime(t: Transfer): string {
  ...
  return ({ '.png': 'image/png', '.jpg': 'image/jpeg', '.pdf': 'application/pdf', '.zip': 'application/zip' } as Record<string, string>)[ext]
